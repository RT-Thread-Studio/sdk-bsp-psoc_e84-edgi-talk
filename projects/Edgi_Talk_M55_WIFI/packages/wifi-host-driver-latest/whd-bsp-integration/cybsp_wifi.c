...
#define CY_WIFI_THREAD_PRIORITY             (CY_RTOS_PRIORITY_REALTIME)
#else
...
//whd_gpio_t mtb_hal_gpio_t host_wake_pin
        result = _cybsp_wifi_sdio_card_init(sdio_obj);
        if (result == CY_RSLT_SUCCESS)
        {
            // whd_gpio_t host_oob_pin=
            // {
            //     .pin_num =host_wake_pin->pin_num,
            //     .port_num = host_wake_pin->port_num,
            //     .port_addr = host_wake_pin->port_addr,
            //     .callback_data = host_wake_pin->callback_data
            // };
            // whd_oob_config_t sdio_oob_config =
            // {
            //     .host_oob_pin      = host_oob_pin,
            //     .dev_gpio_sel      = DEFAULT_OOB_PIN,
            //     .is_falling_edge   = (CY_WIFI_HOST_WAKE_IRQ_EVENT == MTB_HAL_GPIO_IRQ_FALL)
            //         ? WHD_TRUE
            //         : WHD_FALSE,
