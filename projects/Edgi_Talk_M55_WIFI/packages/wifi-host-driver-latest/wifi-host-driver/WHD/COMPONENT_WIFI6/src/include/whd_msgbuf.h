...
#define WHD_IOCTL_PACKET_TIMEOUT      (20000)     /* IOCTL get buffer Timeout */
#define WHD_IOCTL_TIMEOUT_MS          (10000)     /** Need to give enough time for coming out of Deep sleep (was 5000) */
#define WHD_IOCTL_NO_OF_RETRIES       (20)        /** If the IOCTL reponse not received, do these number of retry **/
...
#define WHD_MSGBUF_IOCTL_MAX_TX_SIZE           (2048)    /* Should be less than 2KB(IOCTL inbut Buffer) */
#define WHD_MSGBUF_IOCTL_MAX_RX_SIZE           (8192)
#define WHD_MSGBUF_EVENT_MAX_RX_SIZE           (2048)

#define WHD_MSGBUF_DATA_MAX_RX_SIZE            (4096 - sizeof(whd_buffer_header_t))

#define WHD_MSGBUF_MAX_IOCTLRESPBUF_POST        5
#define WHD_MSGBUF_MAX_EVENTBUF_POST            5

#define WHD_MSGBUF_RXBUFPOST_TIMER_DELAY        100000    /* RX buffer Timer expiry timeout in milliseconds */
...
#define WHD_MAX_TXSTATUS_WAIT_RETRIES           20

#define WHD_EVENT_HANDLER_LIST_SIZE             (10)  /** Maximum number of simultaneously registered event handlers */
